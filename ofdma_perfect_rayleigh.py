...
# ===========================================================================
print("\n" + "="*60)
print("  SECTION 5 – Channel + Receiver  (perfect CSI, Rayleigh)")
print("="*60)
...
for s in range(num_ofdm_symbols):
    ...
    h_channel     = (np.random.randn() + 1j * np.random.randn()) / np.sqrt(2)
...
h_abs2_series = np.abs(np.array(h_channel_series)) ** 2

window_tx_precomp = np.hanning(len(tx_signal_precomp))
spectrum_tx_precomp = np.fft.fftshift(np.fft.fft(tx_signal_precomp * window_tx_precomp, 4096))

Es_data = 1.0
Eb_user = (symbols_per_user_per_sym * Es_data * (1 + cp_length / sub)) / bits_per_symbol_ue1

gamma_bar_series = Eb_user / noise_var_series
gamma_inst_series = h_abs2_series * gamma_bar_series
ebn0_bar_series_db = 10 * np.log10(gamma_bar_series)
...
for i in range(len(ebn0_bins) - 1):
    ...
    ebn0_bin_centers.append(10 * np.log10(gamma_bar_bin))

    ber_bin_ue1.append(np.mean(instant_ber_ue1[mask]))
    ber_bin_ue2.append(np.mean(instant_ber_ue2[mask]))

    ber_rayleigh_bin.append(0.5 * (1 - np.sqrt(gamma_bar_bin / (1 + gamma_bar_bin))))
    ber_exact_bin.append(np.mean(ber_awgn_qpsk_from_gamma(gamma_inst_series[mask])))
...
ber_rayleigh_bin = np.array(ber_rayleigh_bin)
ber_exact_bin = np.array(ber_exact_bin)
...
ber_rayleigh_bin = ber_rayleigh_bin[mask_valid]
ber_exact_bin = ber_exact_bin[mask_valid]
el_bin_mean = el_bin_mean[mask_valid]
sym_bin_count = sym_bin_count[mask_valid]

print(f"  Average Eb/N0 range used for theory: {np.min(ebn0_bar_series_db):.2f} dB to {np.max(ebn0_bar_series_db):.2f} dB")
...
print()
print(f"  {'Eb/N0(dB)':>10}  {'Mean El(deg)':>12}  {'Symbols':>8}  {'UE1 BER':>12}  {'UE2 BER':>12}  {'Rayleigh':>12}  {'ExactAvg':>12}")
print("  " + "-"*96)
for i in range(len(ebn0_bin_centers)):
    print(f"  {ebn0_bin_centers[i]:>10.2f}  {el_bin_mean[i]:>12.2f}  {sym_bin_count[i]:>8d}  {ber_bin_ue1[i]:>12.8f}  {ber_bin_ue2[i]:>12.8f}  {ber_rayleigh_bin[i]:>12.8f}  {ber_exact_bin[i]:>12.8f}")
...
print("Mean |h|^2 =", np.mean(np.abs(np.array(h_channel_series))**2))
print("First 5 h =", h_channel_series[:5])
...
if OFDMA_GRAPHICS_AVAILABLE:
    ...
    spec_tx_orig = np.fft.fftshift(np.fft.fft(tx_signal * np.hanning(len(tx_signal)), 4096))
    spec_tx_prec = np.fft.fftshift(np.fft.fft(tx_signal_precomp * np.hanning(len(tx_signal_precomp)), 4096))

    ax_txcmp.plot(10 * np.log10(np.abs(spec_tx_orig)**2 + 1e-15), label='Original TX')
    ax_txcmp.plot(10 * np.log10(np.abs(spec_tx_prec)**2 + 1e-15), label='TX with Pre-Comp')
...
if OFDMA_GRAPHICS_AVAILABLE:
    ...
    plot_ue1 = np.maximum(ber_bin_ue1, 1e-8)
    plot_ue2 = np.maximum(ber_bin_ue2, 1e-8)
    plot_ray = np.maximum(ber_rayleigh_bin, 1e-8)
    plot_exa = np.maximum(ber_exact_bin, 1e-8)
    ...
    ax_ber.semilogy(ebn0_bin_centers, plot_ray, 'k--', label='Theoretical Rayleigh BER', linewidth=2.0)
    ax_ber.semilogy(ebn0_bin_centers, plot_exa, 'g-.', label='Averaged Exact BER from actual gamma', linewidth=2.0)

    ax_ber.set_xlabel("Effective Average Eb/N0 (dB)")
    ax_ber.set_ylabel("BER")
    ax_ber.set_title("NB-IoT-Like Downlink OFDMA BER vs Effective Eb/N0\n(Perfect CSI Receiver + Rayleigh Channel)")
