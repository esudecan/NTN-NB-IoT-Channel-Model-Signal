...
for snr_db in snr_db_range:
    ...
    for s in range(num_ofdm_symbols):
        ...
        # Symbol-based doppler compensation
        ...
        eq_ue1 = raw_ue1 / (h_est_ue1 + 1e-12)
        eq_ue2 = raw_ue2 / (h_est_ue2 + 1e-12)
    ...
    if snr_db == snr_db_range[-1] and not constellation_example_done:

        # Figure: Constellation Before EQ
        ...
        """
        # Figure: Channel estimate
        fig_he, ax_he = plt.subplots(figsize=(8, 4))
        fig_he.canvas.manager.set_window_title('Fig – Channel Estimate')
        ax_he.plot(nb_idx, np.abs(h_est_nb_last), 'o-', label='|H_est| over NB-IoT band')
        ax_he.plot(nrs_local_last, np.abs(h_pilots_last), 'rs', label='Pilot estimates')
        ax_he.set_xlabel("NB-IoT Local Subcarrier Index")
        ax_he.set_ylabel("Magnitude")
        ax_he.set_title(f"Interpolated Channel Estimate at SNR={snr_db} dB")
        ax_he.grid(True); ax_he.legend()
        plt.tight_layout(); plt.show()
       """
        # Figure: RX Spectrum Before/After Doppler Compensation
        fig_sp2, ax_sp2 = plt.subplots(figsize=(10, 4))
        fig_sp2.canvas.manager.set_window_title('Fig – RX Spectrum')
        ax_sp2.plot(20 * np.log10(np.abs(spectrum_rx_before) /
                                  np.max(np.abs(spectrum_rx_before)) + 1e-12),
                    label="Before Compensation")
        ax_sp2.plot(20 * np.log10(np.abs(spectrum_rx_after) /
                                  np.max(np.abs(spectrum_rx_after)) + 1e-12),
                    label="After Compensation")
        ax_sp2.set_xlabel("Frequency"); ax_sp2.set_ylabel("Magnitude (dB)")
        ax_sp2.set_title("RX Spectrum Before / After Doppler Compensation")
        ax_sp2.grid(True); ax_sp2.legend()
        plt.tight_layout(); plt.show()
...
# ===========================================================================
print("\n" + "="*60)
print("  SECTION 6 – BER List (SNR based)")
print("="*60)

theoretical_ber_range = q_function(np.sqrt(2 * 10 ** (snr_db_range / 10.0)))

print()
print(f"  {'SNR(dB)':>8}  {'UE1 BER':>12}  {'UE2 BER':>12}  {'Teori BER':>12}  {'Gap UE1':>10}")
print("  " + "-"*60)
for i, snr_db in enumerate(snr_db_range):
    gap = simulated_ber_ue1[i] - theoretical_ber_range[i]
    print(f"  {snr_db:>8}  {simulated_ber_ue1[i]:>12.8f}  {simulated_ber_ue2[i]:>12.8f}"
          f"  {theoretical_ber_range[i]:>12.8f}  {gap:>+10.6f}")

print()
best_snr_idx = np.argmin(simulated_ber_ue1)
worst_snr_idx = np.argmax(simulated_ber_ue1)
print(f"  Best SNR={snr_db_range[best_snr_idx]} dB -> UE1={simulated_ber_ue1[best_snr_idx]:.8f}")
print(f"  Worst SNR={snr_db_range[worst_snr_idx]} dB -> UE1={simulated_ber_ue1[worst_snr_idx]:.8f}")
print(f"  Average UE1={np.mean(simulated_ber_ue1):.8f}  UE2={np.mean(simulated_ber_ue2):.8f}")

# ===========================================================================
# SECTION 7 – BER CURVE  (Theory vs UE1 vs UE2)
# ===========================================================================
fig_ber, ax_ber = plt.subplots(figsize=(8, 5))
fig_ber.canvas.manager.set_window_title('Fig – BER Curve')
ax_ber.semilogy(snr_db_range, theoretical_ber_range, 'k--',
                label='Theoretical QPSK BER', linewidth=2)
ax_ber.semilogy(snr_db_range, simulated_ber_ue1, 'bo-',
                label='UE1 Simulated BER', linewidth=1.5, markersize=6)
ax_ber.semilogy(snr_db_range, simulated_ber_ue2, 'ro-',
                label='UE2 Simulated BER', linewidth=1.5, markersize=6)
ax_ber.set_xlabel("SNR (dB)")
ax_ber.set_ylabel("BER")
ax_ber.set_title("NB-IoT-Like Downlink OFDMA BER vs SNR\n"
                 "(ITU-R atmospheric channel + orbital Doppler + 4-pilot LS averaging)")
