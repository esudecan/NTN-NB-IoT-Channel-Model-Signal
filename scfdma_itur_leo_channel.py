...
def apply_time_varying_leo_channel(tx_blocks, channel_symbol, fs, delay_samples):
    rx_blocks = []
    h_series = []
    phase_acc = 0.0
...
def run_simulation():
    ...
    rx_clean, h_series = apply_time_varying_leo_channel(tx_blocks, channel_symbol, fs, delay_samples)
    ...
    for slot in range(num_slots):
        ...
        H_est = slot_blocks[pilot_idx] / (pilot_symbol + 1e-12 )
    ...
    for snr_db in snr_eval:
        ...
        for slot in range(num_slots):
            ...
            for sym in range(symbols_per_slot):
                if sym != pilot_idx:
                    rx_data_syms_loop[data_counter] = slot_blocks[sym] / (H_est + 1e-12)
                    data_counter += 1
    ...
    ax4 = axs[1, 0]
    ax4.semilogy(snr_eval, np.array(ber_vs_snr) + 1e-7, 'ro-', linewidth=2)
