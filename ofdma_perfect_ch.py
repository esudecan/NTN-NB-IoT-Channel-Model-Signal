...
def awgn_with_fixed_power(sig, snr_db, ref_power, fft_size, active_subcarriers):
...
def ber_awgn_qpsk_from_gamma(gamma):
    return 0.5 * erfc(np.sqrt(gamma))

def effective_eb_from_ofdm(bits_per_symbol_user, symbols_per_user_per_sym, cp_length, sub):
    Es_data = 1.0
    return (symbols_per_user_per_sym * Es_data * (1 + cp_length / sub)) / bits_per_symbol_user
...
sub                    = 128
cp_length              = 16
system_active_subcarriers = 64
nb_iot_subcarriers     = 12
num_ofdm_symbols       = 60000
snr_bin_count          = 15
...
# ===========================================================================
# SECTION 5 – CHANNEL + RECEIVER  (perfect CSI + TX pre-comp + semi-analytic theory)
# ===========================================================================
print("\n" + "="*60)
print("  SECTION 5 – Channel + Receiver  (perfect CSI + TX pre-comp)")
print("="*60)
...
doppler_actual_arr  = channel_symbol['dop_arr'].copy()
k_factor_arr        = channel_symbol['k_arr'].copy()

doppler_precomp_arr = doppler_actual_arr.copy()
doppler_residual_arr = doppler_actual_arr - doppler_precomp_arr

tx_blocks_precomp      = []
rx_blocks_after_channel = []
h_channel_series        = []
noise_var_series        = []
bit_errors_ue1          = np.zeros(num_ofdm_symbols, dtype=int)
bit_errors_ue2          = np.zeros(num_ofdm_symbols, dtype=int)

for s in range(num_ofdm_symbols):
    tx_block   = tx_blocks[s]
    fd_actual  = doppler_actual_arr[s]
    fd_precomp = doppler_precomp_arr[s]
    snr_db     = calculated_snr_arr[s]
    k_factor_db = k_factor_arr[s]
    t_block    = np.arange(len(tx_block)) / fs

    k_lin = 10 ** (k_factor_db / 10)
    los   = np.sqrt(k_lin / (k_lin + 1))
    nlos  = (np.random.randn() + 1j * np.random.randn()) / np.sqrt(2 * (k_lin + 1))
    h_channel = los + nlos

    tx_block_prec = tx_block * np.exp(-1j * 2 * np.pi * fd_precomp * t_block)
    tx_blocks_precomp.append(tx_block_prec)

    ch_block = tx_block_prec * np.exp(1j * 2 * np.pi * fd_actual * t_block) * h_channel

    tx_ref_power = np.mean(np.abs(tx_block) ** 2)
    rx_block_channel, noise_var = awgn_with_fixed_power(
        ch_block, snr_db, tx_ref_power, sub, nb_iot_subcarriers
    )

    rx_blocks_after_channel.append(rx_block_channel)
    h_channel_series.append(h_channel)
    noise_var_series.append(noise_var)

tx_signal_precomp = np.concatenate(tx_blocks_precomp)
rx_signal = np.concatenate(rx_blocks_after_channel)

noise_var_series = np.array(noise_var_series)
h_channel_series = np.array(h_channel_series)
h_abs2_series    = np.abs(h_channel_series) ** 2

Eb_user = effective_eb_from_ofdm(
    bits_per_symbol_ue1,
    symbols_per_user_per_sym,
    cp_length,
    sub
)

gamma_bar_series = Eb_user / noise_var_series
gamma_inst_series = h_abs2_series * gamma_bar_series
ebn0_bar_series_db = 10 * np.log10(gamma_bar_series + 1e-30)

window_tx_orig = np.hanning(len(tx_signal))
spec_tx_orig = np.fft.fftshift(np.fft.fft(tx_signal * window_tx_orig, 4096))

window_tx_pre = np.hanning(len(tx_signal_precomp))
spec_tx_pre = np.fft.fftshift(np.fft.fft(tx_signal_precomp * window_tx_pre, 4096))
...
for s in range(num_ofdm_symbols):
    ...
    fd_residual = doppler_residual_arr[s]
    t_rx        = np.arange(len(rx_block)) / fs
    rx_block_comp = rx_block * np.exp(-1j * 2 * np.pi * fd_residual * t_rx)
...
eq_const_ue1 = []
eq_const_ue2 = []
eq_points_ue1_per_symbol = []
eq_points_ue2_per_symbol = []
...
for s in range(num_ofdm_symbols):
    ...
    h_perf = h_channel_series[s]

    eq_ue1 = raw_ue1 / h_perf
    eq_ue2 = raw_ue2 / h_perf
    ...
    eq_points_ue1_per_symbol.append(eq_ue1.copy())
    eq_points_ue2_per_symbol.append(eq_ue2.copy())

eq_points_ue1_per_symbol = np.array(eq_points_ue1_per_symbol)
eq_points_ue2_per_symbol = np.array(eq_points_ue2_per_symbol)
...
for s in range(num_ofdm_symbols):
    ...
    bit_errors_ue1[s] = np.sum(rx_b_ue1_s != bits_tx_ue1[b0:b1])
    bit_errors_ue2[s] = np.sum(rx_b_ue2_s != bits_tx_ue2[c0:c1])

ebn0_min = np.floor(np.min(ebn0_bar_series_db))
ebn0_max = np.ceil(np.max(ebn0_bar_series_db))
ebn0_bins = np.linspace(ebn0_min, ebn0_max, snr_bin_count + 1)
...
for i in range(len(ebn0_bins) - 1):
    if i == len(ebn0_bins) - 2:
        mask = (ebn0_bar_series_db >= ebn0_bins[i]) & (ebn0_bar_series_db <= ebn0_bins[i + 1])
    else:
        mask = (ebn0_bar_series_db >= ebn0_bins[i]) & (ebn0_bar_series_db < ebn0_bins[i + 1])
    ...
    gamma_bar_bin = np.mean(gamma_bar_series[mask])
    gamma_inst_bin = gamma_inst_series[mask]

    n_sym_bin = np.sum(mask)
    n_bits_bin_ue1 = n_sym_bin * bits_per_symbol_ue1
    n_bits_bin_ue2 = n_sym_bin * bits_per_symbol_ue2

    n_err_ue1 = np.sum(bit_errors_ue1[mask])
    n_err_ue2 = np.sum(bit_errors_ue2[mask])

    ebn0_bin_centers.append(10 * np.log10(gamma_bar_bin + 1e-30))
    ber_bin_ue1.append(n_err_ue1 / n_bits_bin_ue1)
    ber_bin_ue2.append(n_err_ue2 / n_bits_bin_ue2)
    ber_theory_bin.append(np.mean(ber_awgn_qpsk_from_gamma(gamma_inst_bin)))
    el_bin_mean.append(np.mean(calculated_el_arr[mask]))
    sym_bin_count.append(n_sym_bin)
    bits_bin_count.append(n_bits_bin_ue1)
    err_bin_ue1.append(n_err_ue1)
    err_bin_ue2.append(n_err_ue2)
    valid_bin_mask_list.append(mask.copy())

ebn0_bin_centers = np.array(ebn0_bin_centers)
ber_bin_ue1 = np.array(ber_bin_ue1)
ber_bin_ue2 = np.array(ber_bin_ue2)
ber_theory_bin = np.array(ber_theory_bin)
el_bin_mean = np.array(el_bin_mean)
sym_bin_count = np.array(sym_bin_count)
bits_bin_count = np.array(bits_bin_count)
err_bin_ue1 = np.array(err_bin_ue1)
err_bin_ue2 = np.array(err_bin_ue2)

MIN_BITS = 10000
mask_valid = bits_bin_count >= MIN_BITS

ebn0_bin_centers = ebn0_bin_centers[mask_valid]
ber_bin_ue1 = ber_bin_ue1[mask_valid]
ber_bin_ue2 = ber_bin_ue2[mask_valid]
ber_theory_bin = ber_theory_bin[mask_valid]
el_bin_mean = el_bin_mean[mask_valid]
sym_bin_count = sym_bin_count[mask_valid]
bits_bin_count = bits_bin_count[mask_valid]
err_bin_ue1 = err_bin_ue1[mask_valid]
err_bin_ue2 = err_bin_ue2[mask_valid]
valid_bin_mask_list = [valid_bin_mask_list[i] for i in range(len(valid_bin_mask_list)) if mask_valid[i]]

print(f"  Mean |fd_actual|   = {np.mean(np.abs(doppler_actual_arr)):.2f} Hz")
print(f"  Mean |fd_precomp|  = {np.mean(np.abs(doppler_precomp_arr)):.2f} Hz")
print(f"  Mean |fd_residual| = {np.mean(np.abs(doppler_residual_arr)):.6f} Hz")
print(f"  Max  |fd_residual| = {np.max(np.abs(doppler_residual_arr)):.6f} Hz")
print(f"  Mean |h|^2         = {np.mean(h_abs2_series):.6f}")
print(f"  Effective Eb/N0 range = {np.min(ebn0_bar_series_db):.2f} dB to {np.max(ebn0_bar_series_db):.2f} dB")
print(f"  Overall UE1 BER = {ber_ue1_total:.8f}")
print(f"  Overall UE2 BER = {ber_ue2_total:.8f}")

if OFDMA_GRAPHICS_AVAILABLE:
    fig_txcmp, ax_txcmp = plt.subplots(figsize=(10, 4))
    fig_txcmp.canvas.manager.set_window_title('Fig – TX Spectrum Comparison')
    ax_txcmp.plot(10 * np.log10(np.abs(spec_tx_orig) ** 2 + 1e-15), label='Original TX')
    ax_txcmp.plot(10 * np.log10(np.abs(spec_tx_pre) ** 2 + 1e-15), label='TX with Pre-Comp')
    ax_txcmp.set_xlabel("Frequency Bin")
    ax_txcmp.set_ylabel("Power (dB)")
    ax_txcmp.set_title("TX Spectrum: Before vs After Doppler Pre-Compensation")
    ax_txcmp.grid(True)
    ax_txcmp.legend()
    plt.tight_layout()
    plt.show()
    ...
    fig_spec.canvas.manager.set_window_title('Fig – RX Spectrum')
    ax1.plot(10 * np.log10(np.abs(spectrum_rx_before) ** 2 + 1e-15), color='red')
    ax1.set_title("RX Spectrum: AFTER Channel and TX Pre-Comp")
    ax1.grid(True)
    ax2.plot(10 * np.log10(np.abs(spectrum_rx_after) ** 2 + 1e-15), color='green')
    ax2.set_title("RX Spectrum: AFTER Residual Doppler Compensation")
    ax2.grid(True)
    plt.tight_layout()
    plt.show()
    ...
    ax_cb.plot(np.real(raw_const_ue1), np.imag(raw_const_ue1), 'bo', alpha=0.35, label='UE1 Before EQ')
    ax_cb.plot(np.real(raw_const_ue2), np.imag(raw_const_ue2), 'ro', alpha=0.35, label='UE2 Before EQ')
    ax_cb.set_xlabel("Inphase")
    ax_cb.set_ylabel("Quadrature")
...
if len(valid_bin_mask_list) > 0:
    best_mask = valid_bin_mask_list[-1]

    eq_best_ue1 = eq_points_ue1_per_symbol[best_mask].reshape(-1)
    eq_best_ue2 = eq_points_ue2_per_symbol[best_mask].reshape(-1)

    fig_eq_best, ax_eq_best = plt.subplots(figsize=(6, 6))
    fig_eq_best.canvas.manager.set_window_title('Fig – Constellation After EQ (Highest Eb/N0 Bin)')
    ax_eq_best.plot(eq_best_ue1.real, eq_best_ue1.imag, 'bo', alpha=0.25, markersize=3, label='UE1 high-Eb/N0 bin')
    ax_eq_best.plot(eq_best_ue2.real, eq_best_ue2.imag, 'ro', alpha=0.25, markersize=3, label='UE2 high-Eb/N0 bin')
    for xr, yi in [(-A,-A), (-A,A), (A,-A), (A,A)]:
        ax_eq_best.plot(xr, yi, 'k+', markersize=14, markeredgewidth=2)
    ax_eq_best.set_xlabel('In-phase')
    ax_eq_best.set_ylabel('Quadrature')
    ax_eq_best.set_title(f'After EQ Constellation – Highest Eb/N0 Bin ({ebn0_bin_centers[-1]:.2f} dB)')
    ax_eq_best.grid(True)
    ax_eq_best.axis('equal')
    ax_eq_best.legend()
    plt.tight_layout()
    plt.show()
...
# ===========================================================================
print("\n" + "="*60)
print("  SECTION 6 – BER List (semi-analytic theory)")
print("="*60)

print()
print(f"  {'Eb/N0(dB)':>10}  {'Mean El(deg)':>12}  {'Symbols':>8}  {'Bits':>10}  {'ErrUE1':>8}  {'ErrUE2':>8}  {'UE1 BER':>12}  {'UE2 BER':>12}  {'Theory':>12}")
print("  " + "-"*118)
for i in range(len(ebn0_bin_centers)):
    print(f"  {ebn0_bin_centers[i]:>10.2f}  {el_bin_mean[i]:>12.2f}  {sym_bin_count[i]:>8d}  {bits_bin_count[i]:>10d}  {err_bin_ue1[i]:>8d}  {err_bin_ue2[i]:>8d}  {ber_bin_ue1[i]:>12.8f}  {ber_bin_ue2[i]:>12.8f}  {ber_theory_bin[i]:>12.8f}")

print()
print(f"  Overall UE1 BER={ber_ue1_total:.8f}")
print(f"  Overall UE2 BER={ber_ue2_total:.8f}")
print(f"  Average effective Eb/N0={np.mean(ebn0_bar_series_db):.2f} dB")
print(f"  Minimum effective Eb/N0={np.min(ebn0_bar_series_db):.2f} dB")
print(f"  Maximum effective Eb/N0={np.max(ebn0_bar_series_db):.2f} dB")
...
if OFDMA_GRAPHICS_AVAILABLE:
    ...
    plot_ue1 = ber_bin_ue1.copy()
    plot_ue2 = ber_bin_ue2.copy()
    plot_th  = ber_theory_bin.copy()

    plot_ue1[err_bin_ue1 == 0] = np.nan
    plot_ue2[err_bin_ue2 == 0] = np.nan

    ax_ber.semilogy(ebn0_bin_centers, plot_ue1, 'bo-', label='UE1 Simulated BER', linewidth=1.5, markersize=6)
    ax_ber.semilogy(ebn0_bin_centers, plot_ue2, 'ro-', label='UE2 Simulated BER', linewidth=1.5, markersize=6)
    ax_ber.semilogy(ebn0_bin_centers, plot_th, 'k--', label='Semi-analytic Theoretical BER', linewidth=2.0)

    ax_ber.set_xlabel("Effective Average Eb/N0 (dB)")
    ax_ber.set_ylabel("BER")
    ax_ber.set_title("NB-IoT-Like Downlink OFDMA BER vs Effective Eb/N0\n(Perfect CSI + TX Pre-Comp + Time-Varying Rician-Like Channel)")
